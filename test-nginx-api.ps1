...
Write-Host "1. Проверка здоровья API через nginx..." -ForegroundColor Yellow
try {
    $health = Invoke-RestMethod -Uri "https://$domain/api-holders-health/" -Method Get
    Write-Host "   Status: $($health.status)" -ForegroundColor Green
    Write-Host "   Service: $($health.service)" -ForegroundColor Green
}
catch {
    Write-Host "   Ошибка: $_" -ForegroundColor Red
    Write-Host "   Попробуйте: http://$domain/api-holders-health/" -ForegroundColor Yellow
}
...
try {
    $response = Invoke-RestMethod -Uri "https://$domain/api-holders/holders/$mint" -Method Get
    ...
    Write-Host "   Попробуйте: http://$domain/api-holders/holders/$mint" -ForegroundColor Yellow
}
...
Write-Host "  Health: https://$domain/api-holders-health/" -ForegroundColor White
Write-Host "  Holders: https://$domain/api-holders/holders/$mint" -ForegroundColor White
